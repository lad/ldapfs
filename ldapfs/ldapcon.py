...
class Connection(object):
    ...
    def get(self, host, dn, attrsonly=False):
        """Retrieve the LDAP obejct at the given DN exists on the given server."""
        LOG.debug('ENTER: host={} dn={} attrsonly={}'.format(host, dn, attrsonly))
        return self._search(host, dn, ldap.SCOPE_BASE, attrsonly)


    def search(self, host, dn, recur=False, attrsonly=False):
        """Search for the LDAP obejcts at the given DN exists on the given server."""
        LOG.debug('ENTER: host={} dn={} recur={} attrsonly={}'.format(host, dn, recur, attrsonly))
    ...
    def _search(self, host, dn, scope, attrsonly):
        ...
        try:
            ...
            raise NoSuchObject('No object found at host={} DN={}'.format(host, dn))
